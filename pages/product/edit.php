...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($category_id) || empty($price) || empty($stock)) {
        ...
        // Update produk
        if (updateProduct($conn, $id, $name, $category_id, $price, $stock, $description)) {
            $message = "Produk berhasil diperbarui!";
            $product = getProductById($conn, $id); // Refresh data
        } else {
        ...
        <form action="" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full px-3 py-2 border rounded-md"><?php echo $product['description']; ?></textarea>
            </div>
